...
class DoctorTests(unittest.TestCase):
    ...
    def test_interrupted_transaction_blocks_and_exposes_recovery_json(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(result["recovery"]["classification"], "no_receipted_mutations")
            self.assertEqual(result["recovery"]["decision"], "manual_review")
            self.assertFalse(result["recovery"]["automatic_mutation"])
