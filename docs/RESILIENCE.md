...
Issue #12 still tracks the mutating reconciliation layer. Before a general recovery command is enabled, pre-mutation journaling must still cover owned paths/source installs and GNOME-extension mutations, and the newly journaled external APT-repository, Flatpak and service intents need privacy-reviewed read-only classification against the real machine. Until those proofs exist, an interrupted transaction remains blocked and MacUbuntu will not reconstruct ownership, restore the backup or roll back from guesses.
