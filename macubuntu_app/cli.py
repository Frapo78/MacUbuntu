...
def build_parser(language: str | None = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lang", choices=["auto", "it", "en"], default="auto", help=t("help_lang"))
    parser.add_argument("--dry-run", action="store_true", help=t("help_dry_run"))
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")

    sub = parser.add_subparsers(dest="command", required=True)
    sub.add_parser("audit", help=t("help_audit"))
    sub.add_parser("plan", help=t("help_plan"))
    sub.add_parser("status", help=t("help_status"))
...
def _print_audit(data: dict[str, Any], t: Translator, verbose: bool) -> None:
    osinfo = data["os"]
    pretty = osinfo.get("pretty_name") or "Ubuntu"
...
def _print_plan(data: dict[str, Any], t: Translator, verbose: bool) -> None:
    ...
    if verbose:
        ...
        print(f"{t('technical_details')}:")
        print(
            f"  summary: install={s['install']} set={s['set']} "
            f"keep={s['keep']} skip={s['skip']}"
        )
...
def _print_uninstall(data: dict[str, Any], t: Translator, verbose: bool) -> None:
    ...
    if kept:
        print(f"! {t('uninstall_partial')}")
    else:
        print(f"✓ {t('uninstall_done')}")
    if data.get("dry_run"):
        print(t("dry_run"))
    if verbose:
        _print_result_details(data, t)
...
def _print_result_details(data: dict[str, Any], t: Translator) -> None:
    ...
    for item in data.get("results", []):
        ...
        localized = t(f"result_{status}") if status in {
            "changed", "installed", "restored", "removed", "kept", "skipped",
            "already_converged", "cleared"
        } else status
...
def _print_status(data: dict[str, Any], t: Translator, verbose: bool) -> None:
    print(f"✓ {t('profile_applied') if data['profile_applied'] else t('profile_not_applied')}")
    print(f"✓ {t('converged') if data['converged'] else t('not_converged')}")
